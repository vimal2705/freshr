...
export const MapMarker = ({ shoulddrag=true, gender,setSearchLocation= null, active=false, data= null, animated = false, reference=null, coordinate, onPress, isSelected, isBarber=false, isFacility=false, showCallout=true, calloutContent=3,delivery=false, ...restProps}) => {
  ...
  let icon = null
  if (isFacility) {
    icon = <View style={{...style, width: 28 }}><FontAwesome name="home" size={18} color={isSelected && gender == "female"?"#FA8073" : isSelected && gender == "Both"? "#fff" : isSelected && gender == "male" ? '#0096FF' :  gender == "male"?  "#0096FF" :gender == "female"?"#FA8072":gender == "Both" ? "white" : theme.colors.brand.secondary } /></View>
  }
  if (isBarber) {
